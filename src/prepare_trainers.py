...
def prep_trainer_df(df: pd.DataFrame) -> pd.DataFrame:
    '''
    ...
    df['is_future'] = df['subtypes'].apply(lambda x: 1 if 'Future' in x else 0)
    df['is_ancient'] = df['subtypes'].apply(lambda x: 1 if 'Ancient' in x else 0)
    ...
    # Drop rows where subtypes could not be parsed
    df = df.dropna(subset=['subtypes']).reset_index(drop=True)
